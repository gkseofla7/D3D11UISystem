...
void Graphics::InitShaders(ComPtr<ID3D11Device> &device) {
    ...
    D3D11Utils::CreateVertexShaderAndInputLayout(
        device, L"UIButtonVS.hlsl", samplingIED, uibuttonVS, samplingIL);
...
void Graphics::InitPipelineStates(ComPtr<ID3D11Device> &device) {
    ...
    // UIPSO
    uiPSO.m_vertexShader = samplingVS;
    uiPSO.m_inputLayout = samplingIL;
    uiPSO.m_pixelShader = samplingPS;
    uiPSO.m_rasterizerState = postProcessingRS;
